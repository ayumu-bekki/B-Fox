...
BleVoltageCharacteristic::BleVoltageCharacteristic(
    ...
    : BleCharacteristicInterface(),
      ...
      property_(property),
      blerdf_beacon_interface_(blerdf_beacon_interface) {}

void BleVoltageCharacteristic::Read(std::vector<uint8_t> *const data) {
  BleRDFBeaconInterfaceSharedPtr blerdf_beacon = blerdf_beacon_interface_.lock();
  if (!blerdf_beacon) {
    return;
  }
  ...
  *view = static_cast<int16_t>(blerdf_beacon->GetBatteryVoltage() * 100);
...
void BleBeaconSettingCharacteristic::Write(
  ...
  const uint16_t major = *reinterpret_cast<const uint16_t *>(
      data->data() + 1 + device_name_length);
...
void BleBeaconSettingCharacteristic::Read(std::vector<uint8_t> *const data) {
  ...
  BeaconSettingConstSharedPtr setting = blerdf_beacon->GetSetting().lock();
  if (!setting) {
    return;
  }
...
BleBleRDFService::BleBleRDFService(const uint16_t app_id,
...
void BleBleRDFService::GattsEvent(esp_gatts_cb_event_t event,
...
void BleBleRDFService::AddCharacteristic(
    BleCharacteristicInterfaceSharedPtr bleCharacteristic) {
  characteristics_.push_back(bleCharacteristic);
}

void BleBleRDFService::SetGattsIf(const uint16_t gatts_if) {
  gatts_if_ = gatts_if;
}

uint16_t BleBleRDFService::GetAppId() const { return app_id_; }

uint16_t BleBleRDFService::GetGattsIf() const { return gatts_if_; }
