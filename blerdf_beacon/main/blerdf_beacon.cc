...
BleRDFBeacon::BleRDFBeacon() : voltage_check_task_(), setting_() {}

BleRDFBeacon::~BleRDFBeacon() = default;

void BleRDFBeacon::Start() {
  // Initialize Log
  logger::InitializeLogLevel();

  ESP_LOGI(TAG, "Startup BleRDF Beacon. Version:%s",
...
void BleRDFBeacon::CreateBLEService() {
  // BLE Set Coded Phy
  ...
  // Create BleBleRDFService 347fd67c-9131-4ea0-b0a7-1886d8c0f0df
  ...
  BleServiceInterfaceSharedPtr ble_blerdf_service =
      std::make_shared<BleBleRDFService>(0, service_uuid, 8);
  ble_blerdf_service->AddCharacteristic(ble_voltage_characteristic);
  ble_blerdf_service->AddCharacteristic(ble_beacon_setting_characteristic);

  // create iBeacon Adv data
  BleIBeacon ibeacon_adv_data =
      CreateIBeaconAttr(kBleRDFIBeaconProximityUuid, setting_->GetMajor(),
  ...
  ble_device->AddService(ble_blerdf_service);
...
float BleRDFBeacon::GetBatteryVoltage() const {
  if (voltage_check_task_) {
...
BeaconSettingConstWeakPtr BleRDFBeacon::GetSetting() const { return setting_; }
