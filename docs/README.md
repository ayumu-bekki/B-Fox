...
bfoxは、Bluetooth Low Energy (BLE) を利用し、アマチュア無線方向探知 (ARDF) の遊びを再現・実現するためのオープンソースプロジェクトです。
...
PCBについては、ユニバーサル基板(36x47x1mm)でも作ることが可能です。KiCADの回路図を参考にしてください。
 
PCBWay ([https://www.pcbway.com](https://www.pcbway.com)) などのサービスを利用して基板・ケースの出力が可能です。

- PCB
  - 基板設計情報が含まれています。KiCADで編集が可能です。
    - *_Gerber.zip をそのまま利用できます。(厚さ1mm)
- Case
  - 3Dプリンタで出力できるケースが含まれています。FreeCADで編集が可能です。
    - *.step をそのまま利用できます。
