...
BleVoltageCharacteristic::BleVoltageCharacteristic(
    ...
    : BleCharacteristicInterface(),
      ...
      property_(property),
      brdf_beacon_interface_(brdf_beacon_interface) {}

void BleVoltageCharacteristic::Read(std::vector<uint8_t> *const data) {
  BrdfBeaconInterfaceSharedPtr brdf_beacon = brdf_beacon_interface_.lock();
  if (!brdf_beacon) {
    return;
  }
  ...
  *view = static_cast<int16_t>(brdf_beacon->GetBatteryVoltage() * 100);
...
void BleBeaconSettingCharacteristic::Write(
  ...
  const uint16_t major = *reinterpret_cast<const uint16_t *>(data->data() + 1 + device_name_length);
  const uint16_t minor = *reinterpret_cast<const uint16_t *>(data->data() + 1 + device_name_length + 2);
  const int16_t measured_power = *reinterpret_cast<const int16_t *>(data->data() + 1 + device_name_length + 4);
  const int16_t tx_power = *reinterpret_cast<const int16_t *>(data->data() + 1 + device_name_length + 6);
...
void BleBeaconSettingCharacteristic::Read(std::vector<uint8_t> *const data) {
  ...
  BeaconSettingConstSharedPtr setting = brdf_beacon->GetSetting().lock();
  if (!setting) {
    return;
  }
  ...
  *reinterpret_cast<uint16_t *>(payload.data() + 1 + device_name_length) = setting->GetMajor();
  *reinterpret_cast<uint16_t *>(payload.data() + 1 + device_name_length + 2) = setting->GetMinor();
  *reinterpret_cast<int16_t *>(payload.data() + 1 + device_name_length + 4) = setting->GetMeasuredPower();
  *reinterpret_cast<int16_t *>(payload.data() + 1 + device_name_length + 6) = setting->GetTxPower();
...
BleBRDFService::BleBRDFService(const uint16_t app_id,
...
void BleBRDFService::GattsEvent(esp_gatts_cb_event_t event,
...
void BleBRDFService::AddCharacteristic(
    BleCharacteristicInterfaceSharedPtr bleCharacteristic) {
  characteristics_.push_back(bleCharacteristic);
}

void BleBRDFService::SetGattsIf(const uint16_t gatts_if) {
  gatts_if_ = gatts_if;
}

uint16_t BleBRDFService::GetAppId() const { return app_id_; }
