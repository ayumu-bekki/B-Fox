...
bool Mount() {
  // Mount File System
  ESP_LOGI(kTag, "Mounting FAT filesystem");
  ...
  if (err != ESP_OK) {
    ESP_LOGE(kTag, "Failed to mount FATFS (%s)", esp_err_to_name(err));
    return false;
  }
  return true;
}
...
/// Unmount File System
void Unmount() {
  ESP_LOGI(kTag, "Unmounting FAT filesystem");
...
bool Write(const std::string& file_path, const std::string& body) {
  ...
  if (!file_open_stream.is_open()) {
    ESP_LOGE(kTag, "Failed to open file for writing");
    return false;
  }
...
bool Read(const std::string& file_path, std::string& body) {
  ...
  if (!file_read_stream.is_open()) {
    ESP_LOGE(kTag, "Failed to open file for read");
    return false;
  }
